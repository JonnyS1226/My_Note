...
%求出四个初值
X0=sum(sum(bitand(I1,17)))/(17*SUM) + 0.0000000001;
Y0=sum(sum(bitand(I2,34)))/(34*SUM) + 0.0000000001;
Z0=sum(sum(bitand(I3,68)))/(68*SUM) + 0.0000000001;
H0=sum(sum(bitand(I1,136)))/(136*SUM) + 0.0000000001;
%保留四位小数
...
for i = 1 : 4 * 240 * 240 - 4 * 3000    
    ...
    if total_code(i) >= 11 || total_code(i) <= -9
        ans_code(i) = 0;
        cnt0 = cnt0 + 1;
    else
        ans_code(i) = 1;
