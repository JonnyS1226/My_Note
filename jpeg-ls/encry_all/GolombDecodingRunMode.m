        function [MErrval,totaloutput, code_index]=GolombDecodingRunMode(totaloutput,k, LIMIT,qbpp, code_index, ans_code)
...
%         totaloutput(1:count-1)=[];%取走数据后，压缩数据流中相应元素删除~~~~~~~~~~~~~~~~~~~~~~~~原本计算中对于前半部分的一元编码，只取了0，没有删掉1
        ...
        if(val1<val3)
            ...
            if k~=0 %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~原本没有这个if，但是会出现k等于0的情况
                temp=char();
                for i=1:k
%                     if totaloutput(i) == num2str(ans_code(code_index))
%                         temp(i) = '0';
%                     else
%                         temp(i) = '1';
%                     end
